		...
		$(function(){
			$('.add_btn .del').click(function(){
				$(location).attr("href", "/portfolio/main/shboard/Free/del?no=<?=$vs[0]->no?>");
			});

			$('.add_btn .modify').click(function(){
				$(location).attr("href", "/portfolio/main/shboard/Free/modify?no=<?=$vs[0]->no?>");
			});
		...
		<section>
			<ul>
				...
				<li class="add_input">
					<a href="/static/user/board_files/<?=$vs[0]->filename?>" target="_blank">
						<?=$vs[0]->filename?>
					</a>
				</li>
